...
> **중요**: 2025년 4월부터 Clerk의 **네이티브 Supabase 통합**을 사용합니다. JWT Template은 더 이상 필요하지 않으며, 이 방식이 더 안전하고 간단합니다.
...
3. 3-1에서 복사한 **Clerk domain**을 붙여넣기
4. **"Create connection"** 클릭

**3-3. 통합 확인**

통합이 성공적으로 완료되면:
- Clerk 세션 토큰에 자동으로 `"role": "authenticated"` 클레임이 추가됩니다
- Supabase가 Clerk 토큰을 자동으로 검증합니다
- JWT Template이나 JWT Secret Key 공유가 필요 없습니다

**추가 정보**: 자세한 통합 가이드는 [`docs/clerk-supabase-integration.md`](./docs/clerk-supabase-integration.md)를 참고하세요.
...
3. 다음 마이그레이션 파일들을 순서대로 실행:
   ...
   **5-2. Storage 마이그레이션**
   - `supabase/migrations/setup_storage.sql` 파일 내용을 복사하여 붙여넣기
   - **"Run"** 클릭하여 실행
   - 성공 메시지 확인

   **5-3. 예제 테이블 (선택사항)**
   - `supabase/migrations/setup_instruments_example.sql` 파일 내용을 복사하여 붙여넣기
   - **"Run"** 클릭하여 실행
   - `/instruments` 페이지에서 데이터 조회 테스트 가능

**생성되는 리소스:**
- `users` 테이블: Clerk 사용자와 동기화되는 사용자 정보 테이블
  - RLS 정책이 활성화되어 있으며, `auth.jwt()->>'sub'`로 Clerk user ID 확인
- `uploads` Storage 버킷: 사용자 파일 저장소
  - RLS 정책으로 사용자는 자신의 폴더에만 접근 가능
- `instruments` 테이블 (선택사항): Supabase 연결 테스트용 예제 테이블
  - 공개 읽기 정책으로 설정되어 있어 인증 없이도 조회 가능
...
**테스트 페이지:**
- `/instruments`: Supabase 데이터 조회 예제 (공식 가이드 참고)
...
- SignIn, SignUp, UserButton 등 모든 UI 요소가 한국어로 번역됩니다
- Tailwind CSS v4 호환성 설정 포함

**자세한 내용**: [`docs/clerk-localization.md`](./docs/clerk-localization.md)를 참고하세요.

**참고**: 
- 로컬라이제이션은 Clerk 컴포넌트에만 적용됩니다
- 호스팅된 Clerk Account Portal은 여전히 영어로 표시됩니다
- 이 기능은 현재 실험적(experimental) 상태입니다
...
**중요**: Clerk 네이티브 통합에서는 `auth.jwt()->>'sub'`로 Clerk user ID를 확인합니다.
...
CREATE TABLE your_table (
  ...
  clerk_user_id TEXT NOT NULL,  -- Clerk user ID 저장
  -- 기타 컬럼들
);
...
**자세한 가이드**: [`docs/clerk-supabase-integration.md`](./docs/clerk-supabase-integration.md)의 "RLS 정책 작성 가이드" 섹션을 참고하세요.
